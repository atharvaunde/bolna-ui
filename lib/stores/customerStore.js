...
const useCustomerStore = create((set, get) => ({
    ...
    recalculateRisk: async (customerId) => {
        try {
            ...
            toast.loading("Recalculating risk score...", { id: "recalculate-risk" });
            const response = await api.get(
            ...
            if (response.data.success) {
                ...
                toast.success("Risk score recalculated successfully!", {
                    id: "recalculate-risk",
                    description: `New Risk Level: ${response.data.data.riskLevel} (Score: ${response.data.data.riskScore})`,
                });
            ...
            toast.error(errorMessage, { id: "recalculate-risk" });
            throw error;
        }
    },
    ...
    generateKnowledgebase: async () => {
        try {
            ...
            toast.loading("Generating knowledge base...", { id: "generate-kb" });
            const response = await api.post("/knowledgebase/generate-kb");

            if (response.data.success) {
                set({ isGeneratingKB: false });
                toast.success(
                    "Knowledge base generated and uploaded successfully!",
                    {
                        id: "generate-kb",
                        description: `RAG ID: ${response.data.data?.ragId ?? "—"} · ${response.data.data?.customersProcessed ?? 0} customers · ${response.data.data?.markdownSizeKB ?? 0}KB`,
                        duration: 8000,
                    }
                );
            }
            ...
            set({ isGeneratingKB: false });
            toast.error(errorMessage, { id: "generate-kb" });
            throw error;
        }
    },
    ...
    previewKnowledgebase: async () => {
        try {
            ...
            toast.loading("Loading knowledge base preview...", { id: "preview-kb" });
            ...
            if (response.data.success) {
                ...
                toast.success("Knowledge base preview loaded", {
                    id: "preview-kb",
                    description: `${response.data.data?.customersProcessed ?? 0} customers · ${response.data.data?.markdownSizeKB ?? 0}KB`,
                });
            }
            ...
            toast.error(errorMessage, { id: "preview-kb" });
            throw error;
        }
    },
